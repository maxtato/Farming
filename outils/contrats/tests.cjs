...
function ready(){
 const c=context();
 load(c,['corrigerCommandeMetier','lectureEnCours','quantitePreparation','limiteActionAutomatique','resteObjectifAutomatique','spotPecheProche']);
...
test('Seven new missions preserve the thirty existing ranks and cover every new character',()=>{
 ...
 for(const m of c.MISSIONS.slice(30)){if(m.faire)continue;const s=c.SITES.find(s=>s.nom===m.lieu);assert(m.lignes.length>=(m.lieu==='Scierie'?1:2));for(const l of m.lignes)assert(s.achete.includes(l.cle)||s.accepte.includes(l.cle));}
...
test('5000 generated orders vary customers and quantities, with compatible unique and achievable products',()=>{
 ...
 for(let i=0;i<5000;i++){const o=c.tirerCommande();assert(o);clients.add(o.lieu);const site=c.SITES.find(s=>s.nom===o.lieu);assert(o.lignes.length<=4);assert.equal(new Set(o.lignes.map(l=>l.cle)).size,o.lignes.length);assert.equal(o.temps,0);
  ...
  if(['Menuisier','Conserverie','Criée','Poissonnerie'].includes(o.lieu))assert(o.lignes.length>=2);
 }
...
test('Multi-product buyers retain variety while wood and textile specialists request only their raw material',()=>{const c=equipped(ready());for(const s of c.SITES){if(!s.achete.length&&!s.accepte.length)continue;const cles=[...new Set([...s.achete,...s.accepte])];if(s.nom==='Scierie')assert.deepEqual(cles,['grumes']);else if(s.nom==='Atelier textile')assert.deepEqual(cles,['laine']);else assert(cles.length>=2,s.nom);}});
