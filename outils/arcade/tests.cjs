...
const ctx=vm.createContext({Math:noise,WeakMap,mondePret:true,INTRO:{actif:false},SUSP_LIM:{r:.17,p:.1},ECART_VOIE_MAX:3.5,tempsTrafic:10,
 _gabA:[],_gabB:[],_gabT:[],MACHINES:[],cur:0,TRAFIC:[],impacts:[],AUDIO:{impact(...args){ctx.impacts.push(args);}},secouer(){},siSouvent:()=>false,puff(){}});
const choc=source.slice(source.indexOf('const CHOC ='),source.indexOf('};',source.indexOf('const CHOC ='))+2);
vm.runInContext(choc+'\n'+source.match(/const ETAT_REPOS =[^;]+;/)[0]+'\nconst SON_CIBLES=new WeakMap();\n'+['borner','suspension','directionArcade','enginAuRepos','segSeg','prochesGabarits','right','pivotChoc','choquerEngins','choquerTrafic','freinCarrefour','sonCible'].map(fn).join('\n'),ctx);
...
ctx.gabaritsTrafic=t=>[capsule(t.x+t.ox,t.z+t.oz)];
let passed=0;
